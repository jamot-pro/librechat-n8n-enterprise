...
import { ChevronDown, ChevronUp, Pencil, Check, X, Upload, ArrowLeft, Trash2 } from 'lucide-react';
...
function Section({ title, children, onEdit, editing, onSave, onCancel }: SectionProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800/50">
      ...
      {open && (
        <div className="border-t border-gray-100 px-5 py-4 dark:border-gray-700">{children}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {editing ? (
        ...
        <span className="whitespace-pre-wrap text-sm text-gray-800 dark:text-gray-200">
          {value || '—'}
        </span>
      )}
    </div>
      ...
      <div className="flex flex-col gap-4 p-6">
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field
              label="Company Email"
              value={contactDraft.companyEmail}
              editing={editingContact}
              onChange={(v) => setContactDraft((d) => ({ ...d, companyEmail: v }))}
            />
            <Field
              label="Phone"
            ...
            <Field
              ...
              editing={editingContact}
              onChange={(v) => setContactDraft((d) => ({ ...d, personalEmail: v }))}
            />
            <Field
              label="Address"
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            <Field
              ...
              editing={editingRole}
              onChange={(v) => setRoleDraft((d) => ({ ...d, role: v }))}
            />
            <Field
              label="Skills (comma-separated)"
              value={roleDraft.skills}
              editing={editingRole}
              onChange={(v) => setRoleDraft((d) => ({ ...d, skills: v }))}
            />
          </div>
          <div className="mt-4">
            <TextareaField
              label="Description & Goals"
              value={roleDraft.descriptionGoals}
              editing={editingRole}
              onChange={(v) => setRoleDraft((d) => ({ ...d, descriptionGoals: v }))}
            />
          </div>
        </Section>
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            <Field
              ...
              value={financialDraft.monthlySalary}
              editing={editingFinancial}
              onChange={(v) => setFinancialDraft((d) => ({ ...d, monthlySalary: v }))}
            />
            <Field
              label="Currency"
              value={financialDraft.currency}
              editing={editingFinancial}
              onChange={(v) => setFinancialDraft((d) => ({ ...d, currency: v }))}
            />
          </div>
        </Section>
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            <Field
              ...
              editing={editingSocial}
              onChange={(v) => setSocialDraft((d) => ({ ...d, linkedin: v }))}
            />
            <Field
              label="Instagram"
              value={socialDraft.instagram}
              editing={editingSocial}
              onChange={(v) => setSocialDraft((d) => ({ ...d, instagram: v }))}
            />
            <Field
              label="Twitter / X"
              value={socialDraft.twitter}
              editing={editingSocial}
              onChange={(v) => setSocialDraft((d) => ({ ...d, twitter: v }))}
            />
            <Field
              label="Facebook"
              value={socialDraft.facebook}
              editing={editingSocial}
              onChange={(v) => setSocialDraft((d) => ({ ...d, facebook: v }))}
            />
            <Field
              label="Telegram"
              value={socialDraft.telegram}
              editing={editingSocial}
              onChange={(v) => setSocialDraft((d) => ({ ...d, telegram: v }))}
            />
            <Field
              label="Website"
              value={socialDraft.website}
              editing={editingSocial}
              onChange={(v) => setSocialDraft((d) => ({ ...d, website: v }))}
            />
          </div>
        </Section>
        ...
        <Section
          ...
          onSave={saveDrive}
          onCancel={() => {
            setDriveDraft(candidate.googleDriveFolder ?? '');
            setEditingDrive(false);
          }}
        >
          <Field
